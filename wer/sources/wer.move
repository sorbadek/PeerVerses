...
module wer::auth {
    ...
    public struct Content has key, store {
        ...
        storage_url: Option<String>,   // Walrus storage URL
        content_hash: vector<u8>,      // Content hash for verification
        file_size: u64,                // File size in bytes
        file_type: u8                  // File type (1: image, 2: video, 3: pdf, 4: other)
    }
    ...
    public entry fun create_content_with_storage(
        registry: &mut AuthRegistry,
        content_type: u8,
        title: String,
        description: String,
        ...
        let content = Content {
            ...
            is_approved: false,
            total_engagements: 0,
            storage_url: option::some(storage_url),
            content_hash,
            file_size,
            file_type
        };
    ...
    public entry fun update_content_storage(
        content: &mut Content,
        storage_url: String,
        content_hash: vector<u8>,
        file_size: u64,
        ...
        assert!(tx_context::sender(ctx) == content.creator, EUnauthorizedAccess);
        content.storage_url = option::some(storage_url);
