...
const TutorHubContent = () => {
  ...
  const handleSessionCompletion = async (session: OngoingSession) => {
    ...
    try {
      ...
      const tx = await contractService.awardXP(
        account.address,
        xpEarned,
        `Completed session: ${session.title}`
      );
      ...
      const attendTx = await contractService.attendClass(account.address);
      await suiClient.signAndExecuteTransaction({
        transaction: attendTx,
        requestType: 'WaitForLocalExecution',
      });
