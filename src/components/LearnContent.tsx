...
const LearnContent: React.FC<LearnContentProps> = ({ xpBalance, reputationScore }) => {
  ...
  const handleResourceCompletion = async (resource: LearningResource) => {
    if (!account?.address || !isAuthenticated) {
      toast({
        title: "Error",
        description: "Please connect your wallet and authenticate first",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      // Calculate XP based on resource type and size
      const xpEarned = 100 + // Base XP
        (resource.type === 'video' ? Math.floor(parseInt(resource.duration || '0') / 10) * 5 :
         resource.type === 'pdf' ? (resource.pages || 0) * 2 : 0);

      // Award XP through smart contract
      const tx = await contractService.awardXP(
        account.address,
        xpEarned,
        `Completed ${resource.type}: ${resource.title}`
      );
  ...
  const handleContributeResource = async (newResource: LearningResource) => {
    if (!account?.address || !isAuthenticated) {
      toast({
        title: "Error",
        description: "Please connect your wallet and authenticate first",
    ...
    try {
      // Create content through smart contract
      const contentType = newResource.type === 'video' ? 1 :
                         newResource.type === 'pdf' ? 2 : 3;

      const tx = await contractService.createContent(
        contentType,
        newResource.title,
        newResource.description,
        100 // Base XP reward for creating content
      );
      ...
      setLearningResources(prev => [...prev, { ...newResource, id: prev.length + 1 }]);
      setIsContributeModalOpen(false);

      toast({
        title: "Success",
        description: "Resource contributed successfully!"
      });
    } catch (error) {
      console.error('Error contributing resource:', error);
      toast({
        title: "Error",
        description: "Failed to contribute resource. Please try again.",
        variant: "destructive"
      });
    }
  };
